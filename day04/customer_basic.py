    ...
    if choice=="I":
        ...
        while True:
            while True:
                ...
                if customer['email'].find('@') != -1:               #find: 없으면 -1 반환
                    break
                else:
                    print('이메일 양식으로 입력하세요.')
            check = 0 
            for i in custlist:
                if i['email'] == customer['email']:
                    check = 1                                   #중복값이 있으면 chekc=1이 된다.
                    break                                       #break 는 가장 가까운 반복문에서 탈출(for, while)
            if check == 0:
                break
            print('중복되는 이메일이 있습니다.')

        while True:
            customer['birthyear']=input('생년 >>> ')
        ...
        if page < 0:
            ...
            page= (len(custlist)-1)
            print(f'현재페이지는 {len(custlist)}페이지입니다.')
            print(custlist[page])     
        else:
        ...
        for i in range(len(custlist)):
            if custlist[i]['email']==choice1:
                print(f'{custlist[i]["name"]}님의 정보가 삭제되었습니다.')                          # '' 중복되면 하나는 "" 사용
                del custlist[i]
                delok =1
                break
        if delok==0:
